	...
	methods: {
		...
		restore() {
			...
			reader.onload = function() {
				...
				if (fileContent.gems) vm.gems = fileContent.gems;
				if (fileContent.coins) vm.coins = fileContent.coins;
				if (fileContent.stones) vm.stones = fileContent.stones;
				if (fileContent.itemSelected) vm.itemSelected = fileContent.itemSelected;
				if (fileContent.inventory) vm.inventory = fileContent.inventory;
				if (fileContent.mightTrait) vm.mightTrait = fileContent.mightTrait;
				if (fileContent.deftTrait) vm.deftTrait = fileContent.deftTrait;
				if (fileContent.gritTrait) vm.gritTrait = fileContent.gritTrait;
				if (fileContent.insightTrait) vm.insightTrait = fileContent.insightTrait;
				if (fileContent.auraTrait) vm.auraTrait = fileContent.auraTrait;
				if (fileContent.callingSelected) vm.callingSelected = fileContent.callingSelected;
				if (fileContent.weaponSelected) vm.weaponSelected = fileContent.weaponSelected;
				if (fileContent.armorSelected) vm.armorSelected = fileContent.armorSelected;
				if (fileContent.shieldSelected) vm.shieldSelected = fileContent.shieldSelected;
				if (fileContent.xp) vm.xp = fileContent.xp;
				if (fileContent.characterName) vm.characterName = fileContent.characterName;
				if (fileContent.speciesSelected) vm.speciesSelected = fileContent.speciesSelected;
				if (fileContent.homelandSelected) vm.homelandSelected = fileContent.homelandSelected;
				if (fileContent.historySelected) vm.historySelected = fileContent.historySelected;
				if (fileContent.quirkSelected) vm.quirkSelected = fileContent.quirkSelected;
				if (fileContent.currentHearts) vm.currentHearts = fileContent.currentHearts;
				if (fileContent.brightPoints) vm.brightPoints = fileContent.brightPoints;
				if (fileContent.darkPoints) vm.darkPoints = fileContent.darkPoints;
				if (fileContent.elective1) vm.elective1 = fileContent.elective1;
				if (fileContent.elective2) vm.elective2 = fileContent.elective2;
				if (fileContent.elective4) vm.elective4 = fileContent.elective4;
				if (fileContent.elective6) vm.elective6 = fileContent.elective6;
				if (fileContent.elective8) vm.elective8 = fileContent.elective8;
				if (fileContent.elective10) vm.elective10 = fileContent.elective10;
				if (fileContent.brightGifts) vm.brightGifts = fileContent.brightGifts;
				if (fileContent.darkGifts) vm.darkGifts = fileContent.darkGifts;
				if (fileContent.tenebrateGift) vm.tenebrateGift = fileContent.tenebrateGift;
				if (fileContent.combinationWeaponSelected) vm.combinationWeaponSelected = fileContent.combinationWeaponSelected;
				if (fileContent.weaponCustom) vm.weaponCustom = fileContent.weaponCustom;
				if (fileContent.armorCustom) vm.armorCustom = fileContent.armorCustom;
				if (fileContent.shieldCustom) vm.shieldCustom = fileContent.shieldCustom;
				if (fileContent.customWeaponSelected) vm.customWeaponSelected = fileContent.customWeaponSelected;
				if (fileContent.customArmorSelected) vm.customArmorSelected = fileContent.customArmorSelected;
				if (fileContent.customShieldSelected) vm.customShieldSelected = fileContent.customShieldSelected;
				if (fileContent.weaponNumber) vm.weaponNumber = fileContent.weaponNumber;
				if (fileContent.armorNumber) vm.armorNumber = fileContent.armorNumber;
				if (fileContent.shieldNumber) vm.shieldNumber = fileContent.shieldNumber;
				if (fileContent.companionSelected) vm.companionSelected = fileContent.companionSelected ;
				if (fileContent.followerSpecies) vm.followerSpecies = fileContent.followerSpecies ;
				if (fileContent.followerHumanCalling) vm.followerHumanCalling = fileContent.followerHumanCalling ;
				if (fileContent.followerHumanAbility) vm.followerHumanAbility = fileContent.followerHumanAbility ;
				if (fileContent.followerElfImmunity) vm.followerElfImmunity = fileContent.followerElfImmunity ;
				if (fileContent.followerDarkGift) vm.followerDarkGift = fileContent.followerDarkGift ;
				if (fileContent.soulCompAbility1) vm.soulCompAbility1 = fileContent.soulCompAbility1 ;
				if (fileContent.soulCompAbility5) vm.soulCompAbility5 = fileContent.soulCompAbility5 ;
				if (fileContent.nativeHumanElective) vm.nativeHumanElective = fileContent.nativeHumanElective;
				if (fileContent.languages) vm.languages = fileContent.languages;
				if (fileContent.portraitURL) vm.portraitURL = fileContent.portraitURL;
				if (fileContent.heartsBlade) vm.heartsBlade = fileContent.heartsBlade;
				if (fileContent.wrathsBlade) vm.wrathsBlade = fileContent.wrathsBlade;
			};
